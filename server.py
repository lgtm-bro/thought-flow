...
def update_user(email):
    ...
    updated_user = crud.update_user_info(
        name, email, newEmail, newPassword, currentPassword)
    db.session.commit()
...
def getQuote(keyword):
    ...
    res = requests.get(
        f'https://favqs.com/api/quotes/?filter={keyword}&type=tag', headers=headers)
...
def createSession():
    ...
    new_sess = crud.create_user_session(
        user_id, base_id, second_id, third_id, date)
